...
# class AsyncClusterConnectionPool(ClusterConnectionPool):

#     """ Using _waiting_queue to place async connecion unreaded, which is
#         assigned for brpop
#     """

#     def get_slots_by_keys(self, *keys):
#         slots = {}

#         for key in keys:
#             slots.setdefault(self.nodes.keyslot(key), []).append(key)

#         return slots

#     def get_connections_by_keys(self, *keys):

#         return [self.get_connection_by_key(key) for key in keys]
...
class MultiChannelPoller(object):
    ...
    def _unregister(self, channel, client, conn, type):
        self.poller.unregister(
            self._chan_to_sock[(channel, client, conn, type)])
    ...
    def handle_event(self, fileno, event):
        if event & READ:
            ...
            chan, conn, cmd_type = self._fd_to_chan[fileno]
            chan._poll_error(cmd_type, **{'conn': conn})
...
class Channel(virtual.Channel):
    ...
    def _brpop_start(self, timeout=1):
        ...
        for chan, cli, conn, type in self.connection.cycle.socks:
            if (chan, cli, type) == (self, self.client, "BRPOP"):
                conn.send_command('BRPOP', *_m[conn.node['name']]+[timeout])
    ...
    def _poll_error(self, type, **options):
        if type == 'LISTEN':
            ...
            self.client.parse_response(options['conn'], type)
